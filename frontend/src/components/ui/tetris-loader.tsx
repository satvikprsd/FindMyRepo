...
import { useState, useEffect, useCallback, useRef } from "react"
  ...
  // Tetris pieces - themed colors
const TETRIS_PIECES = [
  // I-piece
  { shape: [[1, 1, 1, 1]], color: 'bg-primary' },
  // O-piece
  { shape: [[1, 1], [1, 1]], color: 'bg-primary/80' },
  // T-piece
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-primary/90' },
  // L-piece
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-primary/70' },
  // S-piece
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-primary/85' },
  // Z-piece
  { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-primary/75' },
  // J-piece
  { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-primary/95' },
]
...
export default function TetrisLoading({ 
  ...
  // Size configurations
  const sizeConfig = {
    sm: { cellSize: 'w-1.5 h-1.5', gridWidth: 4, gridHeight: 8, padding: 'p-0.5' },
    md: { cellSize: 'w-2 h-2', gridWidth: 6, gridHeight: 12, padding: 'p-1' },
    lg: { cellSize: 'w-3 h-3', gridWidth: 8, gridHeight: 16, padding: 'p-1.5' }
  }
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map(cell => ({ ...cell, color: 'bg-primary animate-pulse opacity-50' }))
          }
          return row
        })
    ...
    <div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="text-foreground font-medium transition-colors">{loadingText}</p>
        </div>
      )}
    </div>
  )
